...
def test_options_from_rows_preserves_stable_row_order() -> None:
    generated = options_from_rows(
        [
            {
                schema.KEY_OPTION_ZENITH_ANGLE_DEG: 15.0,
                schema.KEY_OPTION_WAVELENGTH_UM: 1.25,
            },
            {
                schema.KEY_OPTION_ZENITH_ANGLE_DEG: 25.0,
    ...
    assert generated.columns == (
        schema.KEY_OPTION_ZENITH_ANGLE_DEG,
        schema.KEY_OPTION_WAVELENGTH_UM,
    )
...
def test_options_from_rows_uses_explicit_columns_and_broadcast_defaults() -> None:
    generated = options_from_rows(
        ...
        columns=[schema.KEY_OPTION_ZENITH_ANGLE_DEG],
        broadcast={schema.KEY_OPTION_WAVELENGTH_UM: 2.2},
    )

    config = generated.to_table_options_config()

    assert config.columns == [schema.KEY_OPTION_ZENITH_ANGLE_DEG]
    assert config.rows == [[15.0], [25.0]]
    assert config.broadcast == {schema.KEY_OPTION_WAVELENGTH_UM: 2.2}
...
def test_options_from_rows_supports_ragged_ngs_rows_with_null_slots() -> None:
    ...
    generated = options_from_rows(
        [
            ...
            {
                schema.KEY_OPTION_ZENITH_ANGLE_DEG: 30.0,
                "ngs1_r_arcsec": 10.0,
                "ngs1_theta_deg": 90.0,
                "ngs1_mag": 15.0,
                "ngs2_r_arcsec": 20.0,
                "ngs2_theta_deg": 180.0,
                "ngs2_mag": 16.0,
            },
        ],
        columns=columns,
...
def test_options_from_rows_rejects_missing_explicit_column() -> None:
    with pytest.raises(ValueError, match="missing columns"):
        options_from_rows(
            [{schema.KEY_OPTION_ZENITH_ANGLE_DEG: 20.0}],
            columns=[schema.KEY_OPTION_ZENITH_ANGLE_DEG, schema.KEY_OPTION_WAVELENGTH_UM],
        )
...
def test_generated_options_can_initialize_mock_dataset(tmp_path) -> None:
    generated = options_from_rows(
        ...
        broadcast={schema.KEY_OPTION_WAVELENGTH_UM: 1.65},
    )
    ...
    num_sims = init_dataset(
        InitDatasetRequest(
            ...
            setup=SetupConfig(ee_apertures_mas=[50.0]),
