...
from ao_predict.simulation.runner import run_pending_simulations
...
def _success_result(m: int = 3, a: int = 2, ny: int = 4, nx: int = 4) -> SimulationResult:
...
def test_runner_with_simulation_interface(tmp_path):
    ...
    class TiptopSimulation(Simulation):
        ...
        def finalize(self, context: SimulationContext) -> None:
            context.result = _success_result(ny=2, nx=2)
...
def test_runner_with_simulation_interface_filtered_indexes(tmp_path):
    ...
    class TiptopSimulation(Simulation):
        ...
        def prepare_simulation_payload(self, simulation_cfg):
            del simulation_cfg
            return {"name": self.name, "version": self.version, "base_config": "[section]\\nvalue=1\\n"}
